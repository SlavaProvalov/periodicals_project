...
<form action="${pageContext.servletContext.contextPath}/signUp" method="post">
    <c:if test="${errorLogin == 1}">
       <p><i class="error"><fmt:message key="message.loginExistError" bundle="${message}"/></i></p>
    </c:if>
   ...
   <p><b><fmt:message key="user.password" bundle="${lang}"/> :</b></p>
    ...
    <p> <i class="error"><fmt:message key="message.emailExistError" bundle="${message}"/></i></p>
    </c:if>
    <p><b><fmt:message key="user.email" bundle="${lang}"/> :</b></p>
